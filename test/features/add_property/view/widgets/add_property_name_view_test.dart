...
void main() async {
  ...
  group('Test ErrorType data', () {
    test('ensure text extracts correctly', () {
    ...
    test('validate nameIsEmpty check', () {
    ...
    test('validate max length check', () {
      ...
      expect(ErrorType.nameExceed30Chars.check(String.fromCharCodes(List<int>.filled(30, 0))), isFalse);

      // given string is more than 30 characters, then return true
      expect(ErrorType.nameExceed30Chars.check(String.fromCharCodes(List<int>.filled(31, 0))), isTrue);
    },);

    test('validate nameContainsInvalidChars', () {
      // given string with alphabets, space, and comma and dot only, then return false
      expect(ErrorType.nameContainsInvalidChars.check('text and space and comma, and dot.'), isFalse);

      // given string contains number, then return true
      expect(ErrorType.nameContainsInvalidChars.check('text with number 1'), isTrue);

      // given string contains invalid char, then return true
      expect(ErrorType.nameContainsInvalidChars.check('invalid \u002A'), isTrue);
    },);
  });
  ...
  group('verify error scenarios on UI', () {
    testWidgets('when tap text then remove text, then error is visible', (widgetTester) async {
      ...
      // hint text visible
      expect(find.byTextWithColor('Enter property name', const Color(0xFF8C8C8C)), findsOneWidget);
      ...
      expect(find.byTextWithColor('Enter property name', const Color(0xFFD02D2D)), findsOneWidget);
    });

    testWidgets('given error is visible, when enter valid text, then error is gone', (widgetTester) async {
      const Widget widget = AddPropertyNameView();
      ...
      // error text is gone
      expect(find.byTextWithColor('Enter property name', const Color(0xFFD02D2D)), findsNothing);
    });

    testWidgets('when enter invalid text, then error is visible', (widgetTester) async {
      const Widget widget = AddPropertyNameView();
      ...
      // enter invalid text
      await widgetTester.enterText(find.byType(TextField), 'invalid text: there it is');
      await widgetTester.pumpAndSettle();
      // error text is visible
      expect(find.byTextWithColor('Only accept text', const Color(0xFFD02D2D)), findsOneWidget);
    });

    testWidgets('when enter more than 30 characters, then error is visible', (widgetTester) async {
      const Widget widget = AddPropertyNameView();
      ...
      // enter invalid text
      await widgetTester.enterText(find.byType(TextField), 'this is a very long text which will be above 30 characters');
      await widgetTester.pumpAndSettle();
      // error text is visible
      expect(find.byTextWithColor('Maximum 30 characters', const Color(0xFFD02D2D)), findsOneWidget);
    });
